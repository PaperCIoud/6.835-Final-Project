...
public class OffCenterCamera : MonoBehaviour {
	...
	public bool isPast180 = false;
	public bool isDebugging = false;

	//check to see where problems when postive occur
	public bool pastPosNeg = false;
	...
	public void Update () {
		...
		bool showDebugStatement = (transform.position.z < 0 && isDebugging) || (transform.position.z > 0 && pastPosNeg && isDebugging);
		pastPosNeg = transform.position.z < 0;

		//calculate projection
		Matrix4x4 genProjection = GeneralizedPerspectiveProjection(
			BottomLeftCorner, BottomRightCorner,
			TopLeftCorner, transform.position,
			cam.nearClipPlane, cam.farClipPlane, showDebugStatement, wasInverted);
		cam.projectionMatrix = genProjection;
	}

	public void DoInversion() {
		BottomLeftCornerTransform.localPosition = Vector3.Scale(BottomLeftCornerTransform.localPosition, new Vector3(-1, 1, 1));
		TopLeftCornerTransform.localPosition = Vector3.Scale(TopLeftCornerTransform.localPosition, new Vector3(-1, 1, 1));
		BottomRightCornerTransform.localPosition = Vector3.Scale(BottomRightCornerTransform.localPosition, new Vector3(-1, 1, 1));

		transform.localEulerAngles = transform.localEulerAngles + new Vector3(wasInverted? 180: -180, 0, 0);
	}

	private void FixedUpdate()
	{

		if ((transform.position.z > 0 && !wasInverted) || transform.position.z <= 0 && wasInverted) {
			DoInversion();
    ...
    // Off-center camera
    public static Matrix4x4 GeneralizedPerspectiveProjection(Vector3 pa, Vector3 pb, Vector3 pc, Vector3 pe, float near, float far, bool isDebugging = false, bool wasInverted = false){
		...
		left = Vector3.Dot(vr, wasInverted? vb : va)*near/eyedistance;
		//Debug.Log ("left : " + left);
		right  = Vector3.Dot(vr, wasInverted? va : vb)*near/eyedistance;
		//Debug.Log ("right : " + right);
		bottom  = Vector3.Dot(vu, va)*near/eyedistance;
		top = Vector3.Dot(vu, vc)*near/eyedistance;

		if (isDebugging) {
			Debug.Log(string.Format("va: {0}, vb: {1}, vc: {2}, left: {3}, right: {4}, bottom: {5}, top: {6}", va, vb, vc, left, right, bottom, top));
		}
		...
		Matrix4x4 rotate90 = new Matrix4x4(
			new Vector4(1, 0, 0, 0),
			new Vector4(0, 0, -1, 0),
			new Vector4(0, -1, 0, 0),
			new Vector4(0, 0, 0, -1)
		...
		Matrix4x4 flippedIdentity = new Matrix4x4(
			new Vector4(-1, 0, 0, 0),
			new Vector4(0, 1, 0, 0),
			new Vector4(0, 0, 1, 0),
			new Vector4(0, 0, 0, 1)
		...
		transformMatrix = transformMatrix * (wasInverted? flippedIdentity : Matrix4x4.identity);
